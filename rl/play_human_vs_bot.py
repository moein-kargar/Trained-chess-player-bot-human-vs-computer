...
class GameState:
    def __init__(self) -> None:
        ...
        self.board: list[list[str]] = self.board_obj.board
        self.turn: str = "w"

        self.has_moved: dict[str, bool] = {
        ...
        self.valid_moves: list[Square] = []
        self.movegen: MoveGenerator = MoveGenerator()
    ...
    def pos_to_rc(self, pos: tuple[int, int]) -> Square:
        x, y = pos
        return (y // SQUARE_SIZE, x // SQUARE_SIZE)

    def get_valid_moves_for(self, r: int, c: int) -> list[Square]:
        return self.movegen.legal_moves_safe(self.board, r, c, self.en_passant_target, self.has_moved)

    def make_move(self, r0: int, c0: int, r: int, c: int, screen: Optional[pygame.Surface] = None) -> bool:
        ...
        if (
            ...
            and (r, c) == self.en_passant_target
            and c != c0
            and self.board[r][c] == ""
        ):
    ...
    def show_promotion_menu(self, screen: pygame.Surface, color: str) -> Optional[str]:
    ...
    def clone(self) -> GameState:
        # clone for MCTS (no images needed)
        new = GameState.__new__(GameState)
        new.board_obj = None  # type: ignore[assignment]
